...
class ProfileSystemAdmin implements Migratable
{
    public int $id;
    public int $userId;
    ...
    {
        $sql = $pdo->prepare('SELECT * FROM sysads WHERE id = ? LIMIT 1');
        $sql->execute([$id]);
    ...
    {
        $sql = $pdo->prepare('SELECT * FROM sysads WHERE user_id = ? LIMIT 1');
        $sql->execute([$id]);
    ...
    {
        $sql = $pdo->prepare('
            INSERT INTO sysads (user_id, first_name, middle_name, last_name)

...
class PasswordReset implements Migratable
{
    public int $id;
    public int $userId;
    public string $token;
    ...
    {
        $sql = $pdo->prepare('SELECT * FROM password_resets WHERE id = ? LIMIT 1');
        $sql->execute([$id]);
    ...
    {
        $sql = $pdo->prepare('SELECT * FROM password_resets WHERE token = ? LIMIT 1');
        $sql->execute([$token]);

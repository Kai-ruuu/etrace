<?php

namespace App\Models;

use App\Core\Migratable;
use DateTime;
use PDO;

class GraduateRecord implements Migratable
{
    public int $id;
    public int $courseId;
    public bool $archived;
    public int $deanUploaderId;
    public int $graduationYear;
    public string $filename;
    public DateTime $createdAt;
    public DateTime $updatedAt;

    public static function table(): string
    {
        return "graduate_records";
    }

    public static function migrate(PDO $pdo): void
    {
        $table = self::table();
        $sql = $pdo->prepare("CREATE TABLE IF NOT EXISTS {$table}(
            id INT AUTO_INCREMENT PRIMARY KEY,
            course_id INT NOT NULL,
            archived BOOLEAN NOT NULL DEFAULT FALSE,
            dean_uploader_id INT NOT NULL,
            graduation_year INT NOT NULL,
            filename VARCHAR(100) UNIQUE NOT NULL,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
            FOREIGN KEY (course_id) REFERENCES courses(id) ON DELETE CASCADE,
            FOREIGN KEY (dean_uploader_id) REFERENCES deans(id) ON DELETE CASCADE
        );");
        $sql->execute();
    }

    public static function fromRow(array $row): self
    {
        $instance                 = new self();
        $instance->id             = (int) $row['id'];
        $instance->courseId       = (int) $row['course_id'];
        $instance->archived       = $row['archived'];
        $instance->deanUploaderId = (int) $row['dean_uploader_id'];
        $instance->graduationYear = (int) $row['graduation_year'];
        $instance->filename       = $row['filename'];
        $instance->createdAt      = new DateTime($row['created_at']);
        $instance->updatedAt      = new DateTime($row['updated_at']);
        return $instance;
    }

    public static function findAll(PDO $pdo): array
    {
        $sql = $pdo->prepare('SELECT * FROM graduate_records');
        $sql->execute();

        return array_map(fn($row) => self::fromRow($row), $sql->fetchAll());
    }

    public static function findById(PDO $pdo, int $id): ?self
    {
        $sql = $pdo->prepare('SELECT * FROM graduate_records WHERE id = ? LIMIT 1');
        $sql->execute([$id]);

        $row = $sql->fetch();
        return $row ? self::fromRow($row) : null;
    }

    public static function findByBatchAndCourseId(PDO $pdo, int $batch, int $courseId): ?self
    {
        $sql = $pdo->prepare('SELECT * FROM graduate_records WHERE graduation_year = ? AND course_id = ? LIMIT 1');
        $sql->execute([$batch, $courseId]);

        $row = $sql->fetch();
        return $row ? self::fromRow($row) : null;
    }

    public static function create(PDO $pdo, array $data): self
    {
        $sql = $pdo->prepare('
            INSERT INTO graduate_records (
                course_id,
                dean_uploader_id,
                graduation_year,
                filename
            )
            VALUES (:course_id, :dean_uploader_id, :graduation_year, :filename)
        ');

        $sql->execute([
            ':course_id'        => $data['course_id'],
            ':dean_uploader_id' => $data["dean_uploader_id"],
            ':graduation_year'  => $data["graduation_year"],
            ':filename'         => $data["filename"],
        ]);

        return self::findById($pdo, (int) $pdo->lastInsertId());
    }

    public static function update(PDO $pdo, int $id, array $data): ?self
    {
        $sql = $pdo->prepare('
            UPDATE graduate_records
            SET
                filename = :filename,
                archived = :archived
            WHERE id = :id
        ');

        $sql->execute([
            ':filename' => $data['filename'],
            ':archived' => $data['archived'],
            ':id'       => $id,
        ]);

        return self::findById($pdo, $id);
    }

    public function toArray(): array
    {
        return [
            'id'               => $this->id,
            'course_id'        => $this->courseId,
            'archived'         => $this->archived,
            'dean_uploader_id' => $this->deanUploaderId,
            'graduation_year'  => $this->graduationYear,
            'filename'         => $this->filename,
            'created_at'       => $this->createdAt->format('Y-m-d H:i:s'),
            'updated_at'       => $this->updatedAt->format('Y-m-d H:i:s'),
        ];
    }
}
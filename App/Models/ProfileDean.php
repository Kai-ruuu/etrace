...
class ProfileDean implements Migratable
{
    public int $id;
    public int $userId;
    public int $schoolId;
    ...
    {
        $sql = $pdo->prepare('SELECT * FROM deans');
        $sql->execute();
    ...
    {
        $sql = $pdo->prepare('SELECT * FROM deans WHERE id = ? LIMIT 1');
        $sql->execute([$id]);
    ...
    {
        $sql = $pdo->prepare('SELECT * FROM deans WHERE user_id = ? LIMIT 1');
        $sql->execute([$id]);
    ...
    {
        $sql = $pdo->prepare('
            INSERT INTO deans (user_id, school_id, first_name, middle_name, last_name)
            VALUES (:user_id, :school_id, :first_name, :middle_name, :last_name)
        ');

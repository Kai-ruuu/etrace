...
class JobPostLike implements Migratable
{
    public int $id;
    public int $alumniId;
    ...
    {
        $sql = $pdo->prepare('SELECT * FROM job_post_likes WHERE id = ? LIMIT 1');
        $sql->execute([$id]);
    ...
    {
        $sql = $pdo->prepare('SELECT * FROM job_post_likes WHERE alumni_id = ? LIMIT 1');
        $sql->execute([$id]);
    ...
    {
        $sql = $pdo->prepare('SELECT * FROM job_post_likes WHERE alumni_id = ? AND job_post_id = ? LIMIT 1');
    ...
    {
        $sql = $pdo->prepare('
            INSERT INTO job_post_likes (alumni_id, job_post_id)
            VALUES (?, ?)
        ');
    ...
    {
        $sql = $pdo->prepare('DELETE FROM job_post_likes WHERE id = ?');
        $sql->execute([$id]);
